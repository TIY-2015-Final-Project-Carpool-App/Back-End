class CarpoolsController < ApplicationController
  ...
  def index
    if params[:username]
      ...
      # @carpools = JoinedCarpool.where(user_id: @user.id)
      @carpools = Carpool.joins(:joined_carpools).where(joined_carpools: { user_id: @user } )
      if @carpools
        render 'index.json.jbuilder', status: :ok
      else
        render json: { errors: 'No carpools found.' }, status: :no_content
      end
    else
  ...
  def invites
    ...
    if @user
      ...
      if @joined_carpools 
        ...
        render json: { errors: "No unaccepted carpool invitations for this user." }, status: :no_content
      end
    else
  ...
  def join
    ...
    if current_user.access_token == @carpool.creator.access_token
      ...
      if @joined_carpool.save
        render partial: 'joined', status: :created
      else
  ...
  def activate
    @carpool = Carpool.find_by(id: params[:id])
    if @carpool
      @joined_carpool = @carpool.joined_carpools.where(user_id: current_user.id)
      if @joined_carpool && @joined_carpool.activated == false
        if params[:join_token] == @joined_carpool.join_token
          @joined_carpool.activated = true
  ...
  def remove
    ...
    if current_user.access_token == @user.access_token || current_user.access_token == @carpool.creator.access_token
      if @joined_carpool.destroy
        render json: { message: "User removed from carpool." }, status: :no_content
      else
